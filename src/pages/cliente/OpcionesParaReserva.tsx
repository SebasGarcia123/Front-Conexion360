    ...
    const capacidades = Array.from(
      new Set(
        spaces.map((s) => {
          ...
          if (s.capacity >= 2 && s.capacity <= 15) return 'Entre 2 y 15'
          if (s.capacity >= 16 && s.capacity <= 30) return 'Entre 16 y 30'
          return 'Más de 30'
        })
      )
    )
    ...
    return spaces.filter((s) => {
      ...
      if (capacidadTocada) {
        ...
        const ok =
          ...
          (filtroCapacidad.includes('Entre 2 y 15') && cap >= 2 && cap <= 15) ||
          (filtroCapacidad.includes('Entre 16 y 30') && cap >= 16 && cap <= 30) ||
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      ...
      <Box sx={{ display: 'flex', gap: 5, flexGrow: 1, p: 2 }}>
        {/* FILTROS */}
        <Box sx={{ display: 'flex', flexDirection: 'column', py: 3, pr: 3 }}>
